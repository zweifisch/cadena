...
const buildSingleCriteria = (criteria, escape) =>
...
const buildCriteria = (criteria, escape) => {
    if ('$or' in criteria) {
        assert(Array.isArray(criteria.$or), 'OR expected an Array')
        return criteria.$or.map(x => x.$or ? `(${buildCriteria(x, escape)})` : buildCriteria(x, escape)).filter(notEmpty).join(' OR ')
    }
    if ('$and' in criteria) {
        assert(Array.isArray(criteria.$and), 'AND expected an Array')
        return criteria.$and.map(x => x.$or ? `(${buildCriteria(x, escape)})` : buildCriteria(x, escape)).filter(notEmpty).join(' AND ')
    }
    return buildSingleCriteria(criteria, escape)
}
